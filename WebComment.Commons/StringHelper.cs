...
namespace WebComment.Commons
{
    ...
    {
        private static readonly string StaticUrl = ConfigurationManager.AppSettings["StaticUrl"];
        private static Dictionary<string, string> _seoCharacterTable;
        private static readonly object s_lock = new object();

        public static string ParseQueryParamFromUrl(string url, string key)
        {
            url = GetFullLink(url);
            var uriBuilder = new UriBuilder(url);
            return HttpUtility.ParseQueryString(uriBuilder.Query).Get(key);
        }
        ...
        public static string GetFullLink(string link)
        {
            ...
            if (request != null)
                return request.Url.Scheme + System.Uri.SchemeDelimiter + request.Url.Host +
                       (request.Url.IsDefaultPort ? "" : ":" + request.Url.Port) + "/" + link.TrimStart('/');
            else return "http://vienthonga.vn" + "/" + link.TrimStart('/');
        }

        public static string TryGetValue(object obj)
        {
            try
            {
                return obj.ToString();
        ...
        {
            string filename = System.IO.Path.GetFileName(pathImage);
            if (!String.IsNullOrEmpty(filename))
            {
                if (logo)
                {
                    ...
                    return StaticUrl + pathImage.Replace(filename, size + "_" + filename);
                }
                else
                {
                    if (pathImage.Contains("~/Upload"))
