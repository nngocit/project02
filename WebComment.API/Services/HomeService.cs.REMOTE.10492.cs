...
namespace VNPOST.BackEnd.API.Services
{
    public class HomeService : IHomeService
    {
        ...
        private List<ProdectCode> ListProducts
        {
            get
            {
                var keyCacheProductList = "AllProduct";
                var cacheValues = DataCache.GetCache<List<ProdectCode>>(keyCacheProductList);
                if (cacheValues != null)
                {
                    return cacheValues;
                }
        ...
        public DetailModel ProductDetail(int productId)
        {
            ...
            stopwatch.Start();
            var product = GetListProduct(ListProducts.Where(c => c.ProductId == productId).ToList()).FirstOrDefault();
            NLog.LogManager.GetCurrentClassLogger().Debug(" ProductDetail() - product({0}) la {1} giay ", productId, stopwatch.Elapsed.TotalSeconds);  // đã ghi log OK 
            var listColorProduct = GetColorProduct(productId).ToList();
            NLog.LogManager.GetCurrentClassLogger().Debug(" ProductDetail() - listColorProduct({0}) la {1} giay ", productId, stopwatch.Elapsed.TotalSeconds);  // đã ghi log OK 
            ...
            NLog.LogManager.GetCurrentClassLogger().Debug(" ProductDetail() - happyCare({0}) la {1} giay ", productId, stopwatch.Elapsed.TotalSeconds);  // đã ghi log OK 
            var listImage = GetImageByProductId(productId);
            NLog.LogManager.GetCurrentClassLogger().Debug(" ProductDetail() - listImage({0}) la {1} giay ", productId, stopwatch.Elapsed.TotalSeconds);  // đã ghi log OK 
            var listspmuonmua =
                ListProducts.Where(c => GetProductIdMuonmua(productId).ListProductId.Contains(c.ProductId)).ToList();
            NLog.LogManager.GetCurrentClassLogger().Debug(" ProductDetail() - Api listspmuonmua({0}) la {1} giay ", productId, stopwatch.Elapsed.TotalSeconds);  // đã ghi log OK 
            var ListSanphamMuonmua = GetListProduct(listspmuonmua);
            NLog.LogManager.GetCurrentClassLogger().Debug(" ProductDetail() - ListSanphamMuonmua({0}) la {1} giay ", productId, stopwatch.Elapsed.TotalSeconds);  // đã ghi log OK 
        ...
        #region all product
        public List<ProdectCode> ListAllProduct()
        {
        ...
        {
            var rs = ListProducts.Where(c =>
                (rootCategory == 0 || c.ParentId == rootCategory)
                ).Skip(skip).Take(take).ToList();
            return GetListProduct(rs);
        }
        public List<HomeProductVM> HomeProductVM(int skip, int take)
        {
            var data = GetListProduct(ListProducts).ToList();
            return GetHomeProductVM(data.ToList(), skip, take);
        }
