    ...
    public class CommentService : ICommentService
    {
            ...
            try
            {
                ...
                if (commentToUpdate != null)
                {
                    ...
                    if (comment.TrangThaiId != null)
                    {
                        ...
                        commentToUpdate.GhiChu = comment.GhiChu; // Cap nhat ghi chu
                        ...
                        if (commentToUpdate.CM_TrangThaiId == 3)
                        {
                            ...
                            NLog.LogManager.GetCurrentClassLogger().Debug("UpdateComment() - Autotool, assigned comment {0} for SO: {1}", commentId,
                    ...
                    {
                        var currentNVTN = cm_nhanvientiepnhan.Where(x => x.CM_CommentId == commentId && x.PhongBan == comment.PhongBanNV).OrderByDescending(x => x.Id).FirstOrDefault();
                        if (currentNVTN != null)
                        {
                            if (currentNVTN.Status == "A")
                            {
                                ...
                                {
                                    if (commentToUpdate.CM_TrangThaiId == 8 || commentToUpdate.CM_TrangThaiId == 5)
                                    {
                                        currentNVTN.Status = "D";
                                    }
                                }
                        ...
                        else //Truong hop comment do chua chia cho ai
                        {
                            ...
                            //Cap nhat LastAssign cho User
                            var cm_user = _unitOfWork.Repository<User>().Queryable();
                            var user = cm_user.FirstOrDefault(x => x.EmployeeId == comment.MaNVTiepNhan || x.PersonalId == comment.MaNVTiepNhan);
                            if (user != null)
                            {
                                user.LastAssign = DateTime.Now;
                            }
        ...
        public bool PostComment(CommentDataPost comment)
        {
            try
            {
                ...
                if (comment.CommentId != null) //Reply existed comment: Insert data to ReplyComment table
                {
                    ...
                    {
                        NLog.LogManager.GetCurrentClassLogger().Debug("PostComment() - Reply existed comment:: Not found CommentToReply");
                        return false;
                    }
        ...
        public bool UpdateReplyComment(long replyId, UpdateReplyCommentDataPost repcomment)
        {
            try
            {
                ...
                {
                    if (!string.IsNullOrEmpty(repcomment.NoiDung))
                        replyComment.NoiDung = repcomment.NoiDung;
                    if (!string.IsNullOrEmpty(repcomment.Status))
                        replyComment.Status = repcomment.Status.Trim();
        ...
        public bool InsertLoginHistory(UserLoginHistoryDataPost historyPost)
        {
            try
            {
                ...
                var user = cm_User.FirstOrDefault(x => x.EmployeeId == historyPost.MaNV || x.PersonalId == historyPost.MaNV);
                ...
                if (user != null)
                {
                    ...
                    {
                        //find last login
                        var latestLogin = cm_UserLogInHistory.Where(x => x.MaNV.Trim() == historyPost.MaNV.Trim()).OrderByDescending(x => x.Id);
                        if (latestLogin != null)
                        {
                        ...
                        NLog.LogManager.GetCurrentClassLogger().Debug("InsertLoginHistory(): LogOut sucessfully - {0} manv: {1}", historyPost.Type, historyPost.MaNV);
                    }
                    else //Log in
                    {
                        //Tim xem co row nao dang log in ma chua log out
                        var latestLoginWithoutLogOut = cm_UserLogInHistory.Where(x => x.MaNV.Trim() == historyPost.MaNV.Trim() && x.LogOutTime == null && x.LogInTime >= new DateTime(2017, 8, 3)).OrderByDescending(x => x.Id);
                        if (latestLoginWithoutLogOut != null)
                        {
                            var rowToUpdate = latestLoginWithoutLogOut.FirstOrDefault();

                            if (rowToUpdate != null)
                                rowToUpdate.LogOutTime = DateTime.Now.AddMinutes(-1);
                        }
                        ...
                        NLog.LogManager.GetCurrentClassLogger().Debug("InsertLoginHistory(): LogIn sucessfully - {0} manv: {1}", historyPost.Type, historyPost.MaNV);
                    }
        ...
        public SearchCommentResultV2 SearchCommentV2(SearchCommentParamsV2 param, bool isReport = false)
        {
            ...
            try
            {
                ...
                if (param.TrangThaiCM != null)
                {
                    ...
                    if (param.TrangThaiCM == -2)
                    {
                        lstResult = lstResult.Where(x => (x.CM_TrangThaiId == 5) || (x.CM_TrangThaiId == 2) || (x.CM_TrangThaiId == 3) || (x.CM_TrangThaiId == 4) || (x.CM_TrangThaiId == 8) && x.LoaiHienThi == "CM").ToList();
                    }
                    else
                ...
                if (param.MaNVCSTiepNhan != null)
                {
                    //long commentId;
                    //Int64.TryParse(param.CommentId, out commentId);
                    //var needed = _unitOfWork.Repository<CM_NhanVienTiepNhanComment>()
                    //    .Queryable()
                    //    .FirstOrDefault(x => x.CM_CommentId == commentId);
                            ...
                            .Where(
                                x =>
                                    ...
                                    ((x.NgayGioTraLoi != null && x.Status == "D") ||
                                     ...
                                     (x.Comment != null && x.Comment.LoaiHienThi == "DG") ||
                                     (x.NgayGioTraLoi == null && x.Status == "D" && (x.Comment.CM_TrangThaiId == 5) || (x.Comment.CM_TrangThaiId == 8))
                ...
                NLog.LogManager.GetCurrentClassLogger().Debug("SearchCommentV2() Truoc khi ket table {0} giay ", (object)stopwatch.Elapsed.TotalSeconds);
                ...
                if (lstResult.Any())
                {
                    ...
                    foreach (var comment in lstResult)
                    {
                        ...
                        var nvCSTiepNhan = cm_nhanvientiepnhan.FirstOrDefault(x => x.CM_CommentId == comment.Id && x.PhongBan == "CS" && x.NgayGioTraLoi != null);
                        if (nvCSTiepNhan != null)
                        {
                            commentItem.MaNVCSTiepNhan = nvCSTiepNhan.MaNVTiepNhan_VTA;
                            var userInfo =
                                tblUser.FirstOrDefault(
                                    x =>
                                        x.EmployeeId == commentItem.MaNVCSTiepNhan ||
                                        x.PersonalId == commentItem.MaNVCSTiepNhan);
                            if (userInfo != null)
                                commentItem.TenNVCSTiepNhan = userInfo.FullName;
                            commentItem.CM_NVCSTiepNhanId = nvCSTiepNhan.Id;
                        ...
                        else
                        {
                            try
                            {
                                ...
                                {
                                    commentItem.MaNVCSTiepNhan = nvCSTiepNhan.MaNVTiepNhan_VTA;
                                    var userInfo = tblUser.FirstOrDefault(
                                        x =>
                                            x.EmployeeId == commentItem.MaNVCSTiepNhan ||
                                            x.PersonalId == commentItem.MaNVCSTiepNhan);
                                    if (userInfo != null)
                        ...
                        var nvSOTiepNhan = cm_nhanvientiepnhan.FirstOrDefault(x => x.CM_CommentId == comment.Id && x.PhongBan == "SO" && x.NgayGioTraLoi != null);
                        if (nvSOTiepNhan != null)
                        {
                            ...
                            var userInfo =
                                tblUser.FirstOrDefault(
                                    x => x.EmployeeId.Trim() == commentItem.MaNVSOTiepNhan || x.PersonalId.Trim() == commentItem.MaNVSOTiepNhan);
                            if (userInfo != null)
                                commentItem.TenNVSOTiepNhan = userInfo.FullName;
                            commentItem.CM_NVSOTiepNhanId = nvSOTiepNhan.Id;
                        ...
                        else
                        {
                            try
                            {
                                nvSOTiepNhan = cm_nhanvientiepnhan.Where(x => x.CM_CommentId == comment.Id && x.PhongBan == "SO").OrderByDescending(x => x.Id).FirstOrDefault();
                                if (nvSOTiepNhan != null)
                                {
                                    commentItem.MaNVSOTiepNhan = nvSOTiepNhan.MaNVTiepNhan_VTA;
                                    var userInfo =
        ...
        public SearchCommentResultV2 SearchCommentV3(SearchCommentParamsV3 param, bool isReport = false)
        {
            {
                SearchCommentResultV2 lstFinalResult = new SearchCommentResultV2();
                lstFinalResult.ListComments = new List<SearchCommentResult>();
                ...
                try
                {
                    ...
                    if (!string.IsNullOrEmpty(param.where.NgayBatDau))
                    {
                        var tempStart = Convert.ToDateTime(param.where.NgayBatDau);
                        ngayBatDau = new DateTime(tempStart.Year, tempStart.Month, tempStart.Day, 0, 0, 0);
                    }
                    if (!string.IsNullOrEmpty(param.where.NgayKetThuc))
                    {
                        var tempEnd = Convert.ToDateTime(param.where.NgayKetThuc);
                        ngayKetThuc =
                            new DateTime(tempEnd.Year, tempEnd.Month, tempEnd.Day, 0, 0, 0).AddDays(1).AddSeconds(-1);
                    }
                    if (ngayBatDau != null && ngayKetThuc != null)
                    ...
                    //Comment Id
                    if (!string.IsNullOrEmpty(param.where.CommentId))
                    {
                        long commentId;
                        Int64.TryParse(param.where.CommentId, out commentId);
                        if (commentId != 0)
                            lstResult = lstResult.Where(x => x.Id == commentId).ToList();
                        else
                            return lstFinalResult;
                    ...
                    //Ten san pham
                    if (!string.IsNullOrEmpty(param.where.TenSanPham))
                    {
                        lstResult =
                            lstResult.Where(x => x.SP_TenSP.ToLower().Contains(param.where.TenSanPham.ToLower())).ToList();
                    }

                    //Ten NguoiCommentLoaiComment
                    if (!string.IsNullOrEmpty(param.where.NguoiComment))
                    {
                        string decode = HttpUtility.UrlDecode(param.where.NguoiComment);
                        lstResult =
                            lstResult.Where(
                                x =>
                                    !string.IsNullOrEmpty(x.ThongTinKhachHang.HoTen) &&
                    ...
                    //LoaiHienThi
                    if (!String.IsNullOrEmpty(param.where.LoaiHienThi))
                    {
                        lstResult = lstResult.Where(x => x.LoaiHienThi == param.where.LoaiHienThi).ToList();
                    }

                    //TrangThaiComment
                    if (param.where.TrangThaiCM != null)
                    {
                        if (param.where.TrangThaiCM == -1)
                        {
                            lstResult = lstResult.Where(x => x.CM_TrangThaiId != 5 && x.LoaiHienThi == "CM").ToList();
                        }
                        if (param.where.TrangThaiCM == -2)
                        {
                            lstResult =
                                lstResult.Where(
                                    x =>
                                        (x.CM_TrangThaiId == 5) || (x.CM_TrangThaiId == 2) || (x.CM_TrangThaiId == 3) ||
                                        (x.CM_TrangThaiId == 4) || (x.CM_TrangThaiId == 8) && x.LoaiHienThi == "CM")
                                    .ToList();
                        }
                        else

                            lstResult =
                                lstResult.Where(x => x.CM_TrangThaiId == param.where.TrangThaiCM && x.LoaiHienThi == "CM")
                                    .ToList();
                    }


                    //Loai Comment
                    if (param.where.LoaiCommentLevel1 != null)
                    {
                        if (param.where.LoaiCommentLevel1 == -1) //Chua phan loai
                            lstResult = lstResult.Where(x => x.CM_LoaiCommentLevel1Id == null).ToList();
                        else
                            lstResult =
                                lstResult.Where(x => x.CM_LoaiCommentLevel1Id == param.where.LoaiCommentLevel1).ToList();
                    }
                    if (param.where.LoaiCommentLevel2 != null)
                    {
                        lstResult = lstResult.Where(x => x.CM_LoaiCommentLevel2Id == param.where.LoaiCommentLevel2).ToList();
                    }
                    if (param.where.LoaiCommentLevel3 != null)
                    {
                        lstResult = lstResult.Where(x => x.CM_LoaiCommentLevel3Id == param.where.LoaiCommentLevel3).ToList();
                    }
                    if (param.where.LoaiCommentLevel4 != null)
                    {
                        lstResult = lstResult.Where(x => x.CM_LoaiCommentLevel1Id == param.where.LoaiCommentLevel4).ToList();
                    }

                    //Filter again with MaNVCSTiepNhan, 
                    if (param.where.MaNVCSTiepNhan != null)
                    {
                        var listCommentIds =
                            _unitOfWork.Repository<CM_NhanVienTiepNhanComment>()
                                .Queryable()
                                .Where(
                                    x =>
                                        x.MaNVTiepNhan_VTA == param.where.MaNVCSTiepNhan &&
                                        ((x.NgayGioTraLoi != null && x.Status == "D") ||
                                         (x.NgayGioTraLoi == null && x.Status == "A")))
                                .Distinct()
                                .Select(x => x.CM_CommentId)
                    ...
                    //MaNVSOTiepNhan
                    if (param.where.MaNVSOTiepNhan != null)
                    {
                        var listCommentIds =
                            _unitOfWork.Repository<CM_NhanVienTiepNhanComment>()
                                .Queryable()
                                .Where(
                                    x =>
                                        x.MaNVTiepNhan_VTA == param.where.MaNVSOTiepNhan &&
                                        ((x.NgayGioTraLoi != null && x.Status == "D") ||
                                         (x.NgayGioTraLoi == null && x.Status == "A")))
                                .Select(x => x.CM_CommentId)
                                .ToList();
                    ...
                    //PhongBan
                    if (!string.IsNullOrEmpty(param.where.PhongBan))
                    {
                        if (param.where.PhongBan.Trim() == "SO")
                        {
                            lstResult = lstResult.Where(x => x.CM_TrangThaiId == 3 || x.CM_TrangThaiId == 4).ToList();
                        }
                    }
                    ...
                    NLog.LogManager.GetCurrentClassLogger()
                        .Debug("SearchCommentV2() Truoc khi ket table {0} giay ",
                            (object)stopwatch.Elapsed.TotalSeconds);
                    ...
                    if (lstResult.Any())
                    {
                        ...
                        if (param.where.TrangThaiDG == null && !isReport)
                        {
                            // V3 remove totalpage
                            lstFinalResult.TotalPage = (Convert.ToInt32(lstResult.Count % param.limit) != 0)
                                ? (lstResult.Count / param.limit) + 1
                                : (lstResult.Count / param.limit);
                            //lstResult = lstResult.Skip((lstFinalResult.CurrentPage - 1)*20).Take(20).ToList();

                            lstResult = lstResult.Skip(param.skip).Take(param.limit).ToList();
                        }
                        ...
                        foreach (var comment in lstResult)
                        {
                            ...
                            var nvCSTiepNhan =
                                cm_nhanvientiepnhan.FirstOrDefault(
                                    x => x.CM_CommentId == comment.Id && x.PhongBan == "CS" && x.NgayGioTraLoi != null);
                            if (nvCSTiepNhan != null)
                            {
                                commentItem.MaNVCSTiepNhan = nvCSTiepNhan.MaNVTiepNhan_VTA;
                                var userInfo =
                                ...
                                try
                                {
                                    nvCSTiepNhan =
                                        cm_nhanvientiepnhan.Where(
                                            x => x.CM_CommentId == comment.Id && x.PhongBan == "CS")
                                            .OrderByDescending(x => x.Id)
                                            .FirstOrDefault();
                                    if (nvCSTiepNhan != null)
                                    {
                            ...
                            var nvSOTiepNhan =
                                cm_nhanvientiepnhan.FirstOrDefault(
                                    x => x.CM_CommentId == comment.Id && x.PhongBan == "SO" && x.NgayGioTraLoi != null);
                            if (nvSOTiepNhan != null)
                            {
                            ...
                            else
                            {
                                try
                                {
                                    nvSOTiepNhan =
                                        cm_nhanvientiepnhan.Where(
                                            x => x.CM_CommentId == comment.Id && x.PhongBan == "SO")
                                            .OrderByDescending(x => x.Id)
                                            .FirstOrDefault();
                                    if (nvSOTiepNhan != null)
                                    {
                            ...
                            if (isReport)
                            {
                                ...
                                var lstReplies =
                                    cm_reply.Where(
                                        x =>
                                            !string.IsNullOrEmpty(x.MaNV_VTAReply) &&
                                            lstMaNVCS.Contains(x.MaNV_VTAReply) &&
                                            x.CM_CommentId == comment.Id).OrderBy(x => x.NgayGioTao).Take(3).ToList();
                                if (lstReplies != null)
                                {
                                    var lstRepliesList = lstReplies;
                            ...
                            //TrangThaiDG
                            if (param.where.TrangThaiDG != null)
                            {
                                if (param.where.TrangThaiDG == 6) //Da danh gia
                                {
                                    if (commentItem.NgayGioDanhGia != null)
                                        lstFinalResult.ListComments.Add(commentItem);
                                }

                                if (param.where.TrangThaiDG == 7) //Chua danh gia
                                {
                                    if (commentItem.NgayGioDanhGia == null)
                                        lstFinalResult.ListComments.Add(commentItem);
                                }
                    ...
                    if (param.where.TrangThaiDG != null && !isReport)
                    {
                        // V3 remove TotalPage
                        lstFinalResult.ListComments =
                          lstFinalResult.ListComments.Skip(param.skip).Take(param.limit).ToList();
                        //lstFinalResult.ListComments =
                        //    lstFinalResult.ListComments.Skip((lstFinalResult.CurrentPage - 1)*20).Take(20).ToList();
                        ////Count total page
                        lstFinalResult.TotalPage = (Convert.ToInt32(lstResult.Count % param.limit) != 0)
        ...
        public CommentDetailAdmin GetCommentDetailVer2(long Id)
        {
            ...
            try
            {
                ...
                var cm_replycomment =
                    _unitOfWork.Repository<CM_ReplyComment>()
                        .Queryable()
                        .Where(x => x.CM_CommentId == Id && (x.Status == "A" || x.Status == "H"))
                        .OrderBy(x => x.NgayGioTao)
                        .ToList();
                if (cm_comment != null)
                {
                ...
                if (cm_replycomment.Any())
                {
                    //returnCommentDetail.NgayGioTraLoi =  cm_replycomment.OrderByDescending(x => x.NgayGioTao).First().NgayGioTao;
                    var firstVtaReply = cm_replycomment.FirstOrDefault(x => x.MaNV_VTAReply != null);
                    if (firstVtaReply != null)
                        returnCommentDetail.NgayGioTraLoi = firstVtaReply.NgayGioTao;
                }
        ...
        public bool ResetData(ResetDataParam param)
        {
            ...
            try
            {
                ...
                var lstNvTiepNhanCanReset = _unitOfWork.Repository<CM_NhanVienTiepNhanComment>().Queryable().Where(x => x.Status == "A" && x.NgayGioTraLoi == null && (!string.IsNullOrEmpty(param.PhongBan) ? x.PhongBan.Trim() == param.PhongBan.Trim() : 1 == 1)).ToList();

                if (lstNvTiepNhanCanReset != null && lstNvTiepNhanCanReset.Any())
                {
                    foreach (var canreset in lstNvTiepNhanCanReset)
                    {
                        var replied = _unitOfWork.Repository<CM_ReplyComment>().Queryable().FirstOrDefault(x => x.CM_CommentId == canreset.CM_CommentId && !string.IsNullOrEmpty(x.MaNV_VTAReply));
                        if (replied != null)
                        {
                            canreset.NgayGioTraLoi = replied.NgayGioTao;
                            canreset.Status = "D";
                        }
                        else
                        {
                            _unitOfWork.Repository<CM_NhanVienTiepNhanComment>().Delete(canreset);
                            //canreset.Status = "R"; //Reseted
                        }
                    }
                    _unitOfWork.Save();
                    return true;
        ...
        public CommentVtaSite GetListCommentVtaSiteV2(string pageDG, string pageBL, string lastSlut)
        {
            ...
            try
            {
                ...
                var lstReplieDGAndBLs = lstDgAndBlIds != null ? cm_replycomment.Where(x => lstDgAndBlIds.Contains(x.Comment.Id) && x.Status == "A").OrderBy(x => x.NgayGioTao).ToList() : null;
                    ...
                    foreach (var bl in lstBLByUrl)
                    {
                        ...
                        if (lstReplieBLsForCurrentCM != null)
                        {
                            foreach (var rep in lstReplieBLsForCurrentCM)
                            {
                                ...
                                if (reply.TypeQuanTri != "T")
                                {
                                    reply.NoiDung = DetectAndHidePhoneNumber(rep.NoiDung);
                                }
                                else
                                {
                                    reply.NoiDung = rep.NoiDung;
        ...
        public string SearchEmployeeV2()
        {
            List<string> lstEmails = new List<string>();
            List<string> lstNotEmails = new List<string>();
            string result = string.Empty;

            //lstNotEmails.Add("00707");
            ////lstNotEmails.Add("06198");
            ////lstNotEmails.Add("02027");
            ////lstNotEmails.Add("09665");
            ////lstNotEmails.Add("09666");
            ////lstNotEmails.Add("08050");
            ////lstNotEmails.Add("03149");



            //#region init lstEmails
            lstEmails.Add("04897");
            lstEmails.Add("10912");
            ...
            try
            {
                ...
                {
                    var req = new RestRequest("mobileapp-api/GetEmployeeInfoFromCitrix?term=" + email.Replace("@vienthonga.com", String.Empty).Trim().ToLower(), Method.GET) { RequestFormat = DataFormat.Json };
                    var response = client.Execute<HttpContentResult<EmployeeInfoModel>>(req).Data;
                        ...
                        if (resp.Successfully)
                        {
                            ...
                            var user =
                                _unitOfWork.Repository<User>()
                                    .Queryable()
                                    .FirstOrDefault(
                                        x => x.EmployeeId == response.Data.EmpID || x.PersonalId == response.Data.EmpID);
                            if (user != null)
                            {
                                _unitOfWork.Repository<UserInRole>()
                                    .Insert(new UserInRole()
                                    {
                                        UserId = user.Id,
                                        RoleId = "f5ca2a8d-07b8-4eed-8932-aedac7525f8e"
                                    });
                                _unitOfWork.Save();
                            }

                            result = String.Format("SUCCESS: {0}", resp.Message);
                            NLog.LogManager.GetCurrentClassLogger().Debug("Success {0} ", email);
                        }
                        else
                        {

                            NLog.LogManager.GetCurrentClassLogger().Debug("Email : {0} Loi {1} ", email, resp.Message);
                    ...
                    else
                    {
                        ...
                        NLog.LogManager.GetCurrentClassLogger().Debug("lstNotEmails not found: {0} ", email);
            ...
            catch (Exception ex)
            {
                ...
                NLog.LogManager.GetCurrentClassLogger().Debug("Loi not found: {0} ");

            }

            return lstNotEmails.ToString();
        }
        ...
        public List<CM_UserLoginHistory> GetListLogInHistory(string maNV, string startDate, string endDate)
        {
            ...
            try
            {
                ...
                //    returnData = cm_UserLogInHistory.Where(x => x.MaNV == maNV.Trim()).ToList();
                ...
                {
                    var tempStart = Convert.ToDateTime(startDate);
                    ngayBatDau = new DateTime(tempStart.Year, tempStart.Month, tempStart.Day, 0, 0, 0);
                }
                if (!string.IsNullOrEmpty(endDate))
                {
                    var tempEnd = Convert.ToDateTime(endDate);
                    ngayKetThuc =
                        new DateTime(tempEnd.Year, tempEnd.Month, tempEnd.Day, 0, 0, 0).AddDays(1).AddSeconds(-1);
                }
                //var cm_UserLogInHistory = _unitOfWork.Repository<CM_UserLoginHistory>().Queryable().Where(x => x.MaNV.Trim() == maNV.Trim() && x.LogInTime >= ngayBatDau &&   x.LogInTime <= ngayKetThuc);
                var cm_UserLogInHistory = _unitOfWork.Repository<CM_UserLoginHistory>()
                           .Queryable()
                           .Where(x => x.MaNV.Trim() == maNV.Trim() && x.LogInTime >= ngayBatDau && x.LogInTime <= ngayKetThuc)
                           .ToList();
                returnData = cm_UserLogInHistory;
                //if (ngayBatDau != null && ngayKetThuc != null)
                ////    returnData = cm_UserLogInHistory.Where(x => x.LogInTime >= ngayBatDau &&  x.LogOutTime <= ngayKetThuc).ToList();
                //returnData = cm_UserLogInHistory.Where(x => x.LogInTime >= ngayBatDau && x.LogOutTime != null ? x.LogOutTime <= ngayKetThuc : x.LogOutTime == null).ToList();

                //order data again
                if (returnData.Any())
                    returnData = returnData.OrderByDescending(x => x.Id).ToList();
        ...
        public bool VTAReplyCommentV2(NVReplyCommentDataPost comment)
        {
            ...
            try
            {
                ...
                var tblUser = new User();
                tblUser = _unitOfWork.Repository<User>().Queryable().FirstOrDefault(x => x.EmployeeId.Trim() == comment.MaNVReply && (x.PhongBan == "CS" || x.PhongBan == "SO"));
                var mail = new MailHelper();
                string subject = "Bạn đã nhận được phản hồi bình luận từ Viễn Thông A!";
                string body = "";
                string mailTemp = "/Templete/Mailcomment.htm";
                if (mail.ReadEmailTemplate(mailTemp, ref subject, ref body))
                {
                    ...
                    body = body.Replace("[QuanTriVien]", tblUser.FullName);
                    body = body.Replace("[HoTenKhachHang]", string.IsNullOrEmpty(commentToReply.ThongTinKhachHang.GioiTinh) ? "anh/chị " + commentToReply.ThongTinKhachHang.HoTen : (commentToReply.ThongTinKhachHang.GioiTinh == "M" ? "anh " + commentToReply.ThongTinKhachHang.HoTen : "chị " + commentToReply.ThongTinKhachHang.HoTen));
                    body = body.Replace("[Link]", "https://vienthonga.vn/" + commentToReply.SP_URL);
                    body = body.Replace("[NoiDungCauHoi]", commentToReply.NoiDung);
                    body = body.Replace("[GioiTinh]", string.IsNullOrEmpty(commentToReply.ThongTinKhachHang.GioiTinh) ? "anh/chị " : (commentToReply.ThongTinKhachHang.GioiTinh == "M" ? "anh " : "chị "));
                    body = body.Replace("[NoiDungTraLoi]", comment.NoiDung);
                    if (!string.IsNullOrEmpty(commentToReply.ThongTinKhachHang.Email) && commentToReply.ThongTinKhachHang.Email.Contains("@"))
                    {
                        if (mail.SendEmail(commentToReply.ThongTinKhachHang.Email,
                            ...
                            "Bạn đã nhận được phản hồi bình luận từ Viễn Thông A!", body))
                        {
                            NLog.LogManager.GetCurrentClassLogger().Debug("VTAReplyCommentV2() - Gui email thanh cong cho khach hang: {0}", commentToReply.ThongTinKhachHang.Email);
                        }
                        else
                        {
                            NLog.LogManager.GetCurrentClassLogger().Debug("VTAReplyCommentV2() - Khong gui duoc email cho khach hang: {0}", commentToReply.ThongTinKhachHang.Email);
                        }
                    }
                    else
                    {
                        NLog.LogManager.GetCurrentClassLogger().Debug("VTAReplyCommentV2() - Khong ton tai email khach hang de gui mail");
                        return false;
                    }
            ...
            {
                NLog.LogManager.GetCurrentClassLogger().Debug("VTAReplyCommentV2() - Mes: ({0}) ", ex.ToString());
                return false;
            }
        }

        public List<CM_LichLamViec> GetLichLamViec(LichLamViec param)
        {
            List<CM_LichLamViec> lichlvresult = new List<CM_LichLamViec>();
            var cm_lichlamviec =
                   _unitOfWork.Repository<CM_LichLamViec>()
                       .Queryable().ToList();
            if (!cm_lichlamviec.Any())
            {
                if (!string.IsNullOrEmpty(param.MaNV))
                    lichlvresult = cm_lichlamviec.Where(x => x.MaNV == param.MaNV.Trim()).ToList();
            }
